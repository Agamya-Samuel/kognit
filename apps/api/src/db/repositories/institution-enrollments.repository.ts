...
export class InstitutionEnrollmentsRepository extends BaseRepository<InstitutionEnrollment> {
    ...
    try {
      ...
      const [data, totalResult] = await Promise.all([
        this.db
          ...
          .limit(limit)
          .offset(offset),
        this.db.select({ count: institutionEnrollments.id }).from(institutionEnrollments).where(whereClause),
      ]);
  ...
  async count(filters?: { institutionAccountId?: number; studentId?: number; courseId?: number }): Promise<number> {
    try {
      ...
      const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
      const result = await this.db.select({ count: institutionEnrollments.id }).from(institutionEnrollments).where(whereClause);
