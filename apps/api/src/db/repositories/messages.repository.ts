...
import { eq, and, desc, ne, asc, sql, count } from 'drizzle-orm';
...
export class MessagesRepository extends BaseRepository<Message> {
    ...
    try {
      ...
      const [data, totalResult] = await Promise.all([
        this.db
          ...
          .limit(limit)
          .offset(offset),
        this.db.select({ count: count(messages.id) }).from(messages).where(whereClause),
      ]);
  ...
  async count(filters?: { channelId?: number; senderId?: number }): Promise<number> {
    try {
      ...
      const whereClause = and(...conditions);
      const result = await this.db.select({ count: count(messages.id) }).from(messages).where(whereClause);
