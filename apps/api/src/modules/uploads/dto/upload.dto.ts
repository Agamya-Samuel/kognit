...
export class UpdateUploadStatusDto {
  @ApiPropertyOptional({
    enum: ["pending", "uploading", "complete", "failed", "cancelled"],
...
export class UploadProgressResponseDto {
  ...
  @ApiPropertyOptional({
    ...
    description: "S3 signed upload URL",
  })
  @IsString()
  @IsOptional()
  uploadUrl?: string;
  ...
  @ApiPropertyOptional({
    example: "Upload failed due to network error",
    description: "Error message if upload failed",
  })
  @IsString()
  @IsOptional()
...
export class WebhookEventDto {
  ...
  @ApiProperty({
    example: "uploads/course-video.mp4",
  ...
  @ApiProperty({
    example: "ed33a1e8-c8f9-4e03-97d9-069a031ac51b",
    description: "Upload ID from metadata",
  })
  @IsString()
  uploadId: string;
